...
class Client {
    ...
    handshake_onCompleted(protocol, key) {
        this.handler = {
            ...
            26: this.msg_dualCellSwitch.bind(this),
            27: this.msg_dualCellDisable.bind(this),
            28: this.msg_secondarySkin.bind(this),
    ...
    message_onJoin(message) {
        ...
        if (dt < 25 || this.socket.player.hasAnyLinkedCells()) return;
    ...
    message_onSpectate(message) {
        if (message.length !== 1 || this.socket.player.hasAnyLinkedCells())
            return;
    ...
    msg_doubleSplit(message) {
        if (message.length !== 1) return;
        this.queueSplits(2);
    }
    msg_quadSplit(message) {
        if (message.length !== 1) return;
        this.queueSplits(4);
    }
    msg_restart(message) {
    ...
    msg_dualCellSwitch(message) {
        if (message.length !== 1) return;
        this.socket.player.switchDualCell();
    }
    msg_dualCellDisable(message) {
        if (message.length !== 1) return;
        this.socket.player.disableDualControl();
    }
    msg_secondarySkin(message) {
        if (message.length < 2) return;
        const reader = new BinaryReader(message);
        reader.skipBytes(1);
        const skin = this.protocol < 6 ? reader.readStringZeroUnicode() : reader.readStringZeroUtf8();
        this.socket.player.setSecondarySkin(skin);
    }
    ...
    queueSplits(count) {
        this.pendingSplits = Math.min(16, (this.pendingSplits | 0) + Math.max(0, count | 0));
    }
    processMouse() {
        ...
        if (this.mouseData.length === 13) {
            // protocol late 5, 6, 7
            player.inputMouse.x = reader.readInt32() - player.scrambleX;
            player.inputMouse.y = reader.readInt32() - player.scrambleY;
        } else if (this.mouseData.length === 9) {
            // early protocol 5
            player.inputMouse.x = reader.readInt16() - player.scrambleX;
            player.inputMouse.y = reader.readInt16() - player.scrambleY;
        } else if (this.mouseData.length === 21) {
            // protocol 4
            player.inputMouse.x = ~~reader.readDouble() - player.scrambleX;
            player.inputMouse.y = ~~reader.readDouble() - player.scrambleY;
        }
        player.syncActiveMouse?.();
        this.mouseData = null;
    }
    process() {
        ...
        if (this.pendingSplits > 0) {
            const splitCount = Math.min(this.pendingSplits, 16);
            this.pendingSplits = 0;
