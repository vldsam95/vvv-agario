...
class Server {
    constructor() {
        ...
        this.runtimeMtime = {
            ...
            botSettings: Runtime.getMtime(this.runtime.files.botSettings),
            control: Runtime.getMtime(this.runtime.files.control),
        };
        this.lastControlNonce = 0;
        this.ipBanList = [];
        this.minionTest = [];
        this.userList = [];
        this.badWords = [];
    ...
    start() {
        ...
        this.selectedPresetLabel = this.runtime.selectedPreset?.label || this.mode.name;
        // Client Binding
        var bind = this.config.clientBind + "";
        this.clientBind = bind.split(' - ').map((origin) => origin.trim()).filter(Boolean);
    ...
    onHttpServerOpen() {
        ...
        if (this.config.serverEnabled !== false && this.config.serverBots) {
            this.bots.syncPopulation();
            Logger.info("Added " + this.bots.getBotSockets().length + " player bots");
        }
    }
    addNode(node) {
    ...
    getLinkedAvatarColor(baseColor, avatarIndex = 1) {
        const source = [
            baseColor?.r >>> 0 || 0,
            baseColor?.g >>> 0 || 0,
            baseColor?.b >>> 0 || 0,
        ...
        const variants = [
            ...
            [255 - source[0], 255 - source[1], 255 - source[2]],
        ];
        const candidate = variants[Math.abs(avatarIndex - 1) % variants.length];
        if (candidate[0] === source[0] && candidate[1] === source[1] && candidate[2] === source[2]) {
    ...
    getHumanClientCount() {
        return this.clients.reduce((count, socket) => {
            ...
            return count + (socket.isConnected === false ? 0 : 1);
        }, 0);
    }
    compactClientList() {
        this.clients = this.clients.filter((socket) => {
            ...
            if (socket.player.isBot) {
                return socket.isConnected !== false && !socket.isCloseRequest;
            }
            return true;
        });
    }
    createRestartOrphan(sourcePlayer, cells) {
        const orphanSocket = {
            isConnected: false,
            client: null,
            closeTime: this.stepDateTime || Date.now(),
        };
        const orphan = new Player(this, orphanSocket);
        orphan.socket = orphanSocket;
        orphan.configureRestartOrphan(sourcePlayer, cells);
        return orphan;
    }
    respawnControlledPlayer(player, targetPlayer) {
        const controller = player?.getLinkedController ? player.getLinkedController() : player;
        const target = targetPlayer || controller?.getControlledPlayer?.();
        if (!controller || !target || controller.isBot || controller.isMinion || controller.isMi ||
            controller.isRemoved || controller.isRestartOrphan ||
            target.isRemoved || target.isRestartOrphan) {
            return false;
        }
        ...
        if (controller.socket?.client) {
            controller.socket.client.splitRequested = false;
            controller.socket.client.toggleSpectate = false;
            controller.socket.client.ejectRequested = false;
        ...
        controller.multiControl.activePlayer = target;
    ...
    shutdownWorld(reason = "Server disabled by admin") {
        this.disableSpawn = true;
        this.ticks = 0;
        this.startTime = Date.now();
        this.sendChatMessage(null, null, reason);
        for (const socket of this.clients.slice()) {
            try {
                socket.close(1001, "Server disabled");
            } catch (error) {
                // Ignore socket shutdown failures during maintenance transitions.
            }
        }
        this.clients = [];
        this.socketCount = 0;
    ...
    refreshRuntimeFiles() {
        ...
        const configChanged = nextMtime.serverSettings !== this.runtimeMtime.serverSettings ||
            nextMtime.modePresets !== this.runtimeMtime.modePresets ||
            nextMtime.botSettings !== this.runtimeMtime.botSettings;
        this.runtimeMtime = nextMtime;
        if (configChanged) {
            this.applyRuntimeSnapshot(Runtime.loadRuntimeSnapshot());
        }
        this.handleControlCommand();
    }
    applyRuntimeSnapshot(snapshot) {
        const worldResetKeys = [
            "activePreset",
            "serverGamemode",
            "borderWidth",
            "borderHeight",
        ...
        const previousSettings = this.runtime?.serverSettings || {};
        this.runtime = snapshot;
        this.config = snapshot.config;
        this.botSettings = snapshot.botSettings;
        this.modePresets = snapshot.modePresets;
        this.selectedPresetKey = snapshot.selectedPresetKey;
        this.selectedPresetLabel = snapshot.selectedPreset?.label || this.mode?.name || "Arena";
        this.disableSpawn = this.config.serverEnabled === false;
        this.clientBind = String(this.config.clientBind || "")
            .split(" - ")
            .map((origin) => origin.trim())
            .filter(Boolean);
        this.bots.syncPopulation();
        const wasEnabled = previousSettings.serverEnabled !== false;
        const isEnabled = snapshot.serverSettings.serverEnabled !== false;
        if (!wasEnabled && isEnabled) {
            if (this.httpServer) this.resetWorld("Server brought back online");
            return;
        }
        if (wasEnabled && !isEnabled) {
            this.shutdownWorld("Server disabled by admin");
            return;
        }
        const needsWorldReset = worldResetKeys.some((key) => {
            const previousValue = key === "activePreset" ? previousSettings.activePreset : previousConfig[key];
            const nextValue = key === "activePreset" ? snapshot.serverSettings.activePreset : this.config[key];
            return previousValue !== nextValue;
        });
        if (needsWorldReset && this.httpServer && isEnabled) {
            this.resetWorld("Runtime settings updated");
        }
    }
    handleControlCommand() {
        const command = Runtime.readJson(this.runtime.files.control, {nonce: 0, action: "noop"});
        if (!command || !command.nonce || command.nonce === this.lastControlNonce) return;
        this.lastControlNonce = command.nonce;
        switch (command.action) {
            case "reset-world":
                this.resetWorld(command.reason || "Control panel reset");
                break;
            case "broadcast":
                this.sendChatMessage(null, null, String(command.message || "Admin message"));
                break;
        }
    }
    writeRuntimeState() {
        this.compactClientList();
        const humans = this.getHumanClientCount();
        const bots = this.bots.getBotSockets().length;
        Runtime.writeJson(this.runtime.files.state, {
            updatedAt: new Date().toISOString(),
            serverEnabled: this.config.serverEnabled !== false,
            presetKey: this.selectedPresetKey,
            presetLabel: this.selectedPresetLabel,
            mode: this.mode?.name || "Unknown",
            humans,
            bots,
            totalClients: humans + bots,
            scoreLeader: this.leaderboard[0]?._name || null,
            averageUpdateMs: Number(this.updateTimeAvg.toFixed(2)),
            border: {
            ...
            config: {
                ...
                serverMaxConnections: this.config.serverMaxConnections,
                playerMaxCells: this.config.playerMaxCells,
                splitVelocity: this.config.splitVelocity,
    ...
    resetWorld(reason = "Server configuration refreshed") {
        ...
        const previousClients = this.clients.slice();
        const humanSockets = this.clients.filter((socket) =>
            socket &&
            socket.player &&
            !socket.player.isBot &&
            !socket.player.isMinion &&
            !socket.player.isMi &&
            socket.isConnected !== false
        );
        this.sendChatMessage(null, null, reason);
        for (const socket of previousClients) {
            if (socket?.player?.isBot) socket.close();
        }
        this.clients = humanSockets;
        this.socketCount = humanSockets.length;
        ...
        for (const socket of humanSockets) {
            ...
            this.mode.onPlayerSpawn(this, player);
        }
        this.compactClientList();
        this.bots.syncPopulation();
    }
    ...
    getLeaderboardPlayers() {
        ...
        for (const client of this.clients) {
            ...
            const linkedPlayers = typeof player.getLinkedPlayers === "function"
                ...
                : [player];
            for (const candidate of linkedPlayers) {
    ...
    mainLoop() {
        ...
        if (this.config.serverEnabled !== false && this.ticks > this.config.serverRestart && this.run) this.restart();
        if ((this.ticks % 25) === 0) {
            this.refreshRuntimeFiles();
    ...
    movePlayer(cell, client) {
        ...
        const target = client.getControlTargetForCell(cell);
    ...
    spawnPlayer(player, pos, options = {}) {
        if (this.disableSpawn)
            return; // Not allowed to spawn!
        // Check for special starting size
        var size = options.size || this.config.playerStartSize;
        if (player.spawnmass)
        ...
        if (!options.ignoreEjectedSpawn && Math.random() <= this.config.ejectSpawnPercent &&
    ...
    spawnMultiControlAvatar(player) {
        ...
        if (!cell) {
            controller.multiControl.linkedPlayers =
                controller.multiControl.linkedPlayers.filter((item) => item !== avatar);
    ...
    splitCells(client) {
        ...
        const sourceCells = client.getControlledCells();
        var cellToSplit = [];
        for (var i = 0; i < sourceCells.length; i++)
            cellToSplit.push(sourceCells[i]);
    ...
    ejectMass(client, target) {
        if (!this.canEjectMass(client) || client.frozen) return false;
        const sourceCells = client.getControlledCells();
        ...
        for (var i = 0; i < sourceCells.length; i++) {
            ...
            const targetPos = target || client.mouse;
            var d = targetPos.difference(cell.position);
            var sq = d.dist();
            d.x = sq > 1 ? d.x / sq : 1;
            d.y = sq > 1 ? d.y / sq : 0;
    ...
    loadFiles() {
        ...
        // Runtime normalization already converts restart minutes to internal tick thresholds.
    }
